...
const api = {
  ...
  // recomendacoes via neo4j (grafo de ingredientes)
  getSimilares: (drinkId) => request(`${API.recommendation}/similares/${drinkId}`),
};
  ...
  try {
    ...
    appEl.innerHTML = `
      <section class="detail-section">
        ...
        <div class="section" id="similares-section" style="margin-top:2rem">
          <h3 class="section-title">🔗 Drinks Similares <span style="font-size:0.8rem;color:var(--text-muted);font-weight:normal">(via grafo de ingredientes)</span></h3>
          <div id="similares-list"><p class="empty-msg">Buscando recomendações...</p></div>
        </div>
      </section>`;

    // carrega as recomendacoes do neo4j em background
    // se o servico estiver fora do ar a gente so esconde a secao
    carregarSimilares(id);
  } catch (err) {
    showError('Drink não encontrado.');
  }
}

// busca drinks similares no recommendation-service (neo4j)
// se nao tiver nenhum similar ou o servico estiver fora, esconde a secao
async function carregarSimilares(drinkId) {
  const container = document.getElementById('similares-list');
  if (!container) return;
  try {
    const similares = await api.getSimilares(drinkId);
    if (!similares || similares.length === 0) {
      document.getElementById('similares-section').style.display = 'none';
      return;
    }
    container.innerHTML = `<div class="drinks-grid">${similares.map(s => `
      <div class="drink-card" onclick="navigate('/drinks/${s.drinkId}')">
        <div class="card-img-wrap">
          <div class="card-img-placeholder">🍹</div>
        </div>
        <div class="card-body">
          <span class="category-tag">${s.category || 'Outros'}</span>
          <h3>${s.name}</h3>
          <p class="card-desc">${s.ingredientesEmComum} ingrediente${s.ingredientesEmComum > 1 ? 's' : ''} em comum: ${s.ingredientes.slice(0, 3).join(', ')}${s.ingredientes.length > 3 ? '...' : ''}</p>
        </div>
      </div>`).join('')}</div>`;
  } catch (err) {
    document.getElementById('similares-section').style.display = 'none';
